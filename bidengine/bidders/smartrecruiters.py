...
def autofill(field: WebElement, answer):
    ...
    if len(input_elements) > 0:
        ...
        if input.get_attribute("type") == "radio":
            ...
            if len(checkbox_elements) == 1 and answer is True:
                input.click()
    ...
    if len(select_elements) > 0:
        ...
        for (option_index, option_value) in enumerate(option_values):
            if is_correct_answer(option_value, answer):
...
def bid(driver: webdriver.Chrome, url: str, job_id: str):
    form_element = driver.find_element(By.TAG_NAME, "oc-oneclick-form")
    label_elements = form_element.find_elements(By.TAG_NAME, "label")
    unknown_questions = []

    question_index = 0
    while True:
        if question_index == len(label_elements):
            break
        element = label_elements[question_index]
